...
const STEPS: TourStep[] = [
  {
    target: 'tour-header',
    title: 'Status bar',
    body: 'Use this bar to quickly check what you are viewing: terminal name, data vintage, update time, frequency, source, and active model badges. To reopen this walkthrough later, go to Account → User guide.',
    placement: 'bottom',
  },
  {
  ...
  },
  {
    target: 'tour-kpi',
    title: 'KPI ribbon',
    body: 'This ribbon gives you the main readout at a glance: nowcast, recession risk, next-quarter forecast, and certainty range. In Live mode, it shows the latest quarter. In a preset scenario, it updates to the end of that selected window.',
    placement: 'bottom',
  },
  {
    target: 'tour-tabs',
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <h2 id="tour-step-title" className="text-base font-bold mb-2 pr-6">
              {current.title}
            </h2>
            <p className="text-sm leading-relaxed text-muted-terminal mb-4">{current.body}</p>
            ...
            <label className="flex items-center gap-2 cursor-pointer text-xs text-muted-terminal select-none mb-3">
              <input
                type="checkbox"
                checked={neverAgain}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <button
                type="button"
                onClick={finish}
                className="text-xs font-medium px-2 py-1.5 rounded-md text-muted-terminal hover:text-[var(--text)]"
              >
                Skip guide
              </button>
