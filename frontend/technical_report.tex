...
\subsection{Dynamic Time Anchoring}
To ensure the dashboard always reflects the most current state of the economy, we implemented dynamic time anchoring. The application scans the parsed dataset to find the last official published GDP data point, setting it as the anchor ($T_0$). The Nowcast ($T_1$) and Forward Forecast ($T_2$) are then calculated relative to this anchor. This ensures the KPI ribbon and charts always display accurate, context-aware timeframes without hardcoding dates.

\begin{lstlisting}[language=JavaScript, caption=Dynamic Anchor Calculation]
const kpis = useMemo(() => {
  // Find the last index in the selected range that has a valid 'actual' value
  let lastOfficialIdx = timeRange[1];
  while (lastOfficialIdx >= 0 && 
        (processedData[lastOfficialIdx].actual === null || 
         processedData[lastOfficialIdx].actual === undefined)) {
    lastOfficialIdx--;
  }
  
  // Set T_0, T_1, and T_2 relative to the last official data point
  const t0Row = processedData[lastOfficialIdx];
  const tPlus1Row = processedData[Math.min(lastOfficialIdx + 1, processedData.length - 1)];
  const tPlus2Row = processedData[Math.min(lastOfficialIdx + 2, processedData.length - 1)];
  // ...
...
Default charting tooltips are often messy and unformatted. We engineered a custom tooltip component that intercepts the Recharts hover event. It formats the raw decimal data into clean percentages (e.g., \texttt{2.45\%}), aligns the data in a readable table, and color-codes the text to match the lines. This ensures that when a user hovers over a dense cluster of lines, they can instantly parse the exact values. We also engineered dynamic theme inheritance for these tooltips to ensure perfect readability in Dark Mode across all charts.

\subsection{The KPI Ribbon (\texttt{KPIRibbon.tsx})}
The KPI Ribbon provides an at-a-glance summary of the macroeconomic state.
\begin{itemize}
    \item \textbf{Dynamic Context:} It explicitly states the quarter it references (e.g., Q1 2026) based on the dynamic time anchoring, ensuring users always know exactly what period the nowcast applies to.
    \item \textbf{Recession Risk Calculation:} We implemented a statistical Recession Risk gauge. It uses a normal cumulative distribution function (CDF) taking the ensemble model's current nowcast as the mean and deriving the standard deviation from the 95\% confidence interval. This translates raw uncertainty bounds into a highly actionable probability metric.
\end{itemize}

\begin{lstlisting}[language=JavaScript, caption=Recession Risk Probability Calculation]
// Calculate Recession Risk
// 95% CI is roughly mean +/- 1.96 * std
const std = (ciUpper - ciLower) / (2 * 1.96);
const recessionRisk = std > 0 
  ? normalCDF(0, currentNowcast, std) * 100 
  : (currentNowcast < 0 ? 100 : 0);

const riskColor = recessionRisk > 50 ? '#e74c3c' : 
                 (recessionRisk >= 20 ? 'var(--color-brand-warning)' : 'var(--color-brand-success)');
\end{lstlisting}

\subsection{The Ragged Edge Simulator (\texttt{RaggedEdge.tsx})}
Macroeconomic data is released asynchronously throughout the month (the "ragged edge" problem). 
\begin{itemize}
    \item \textbf{Interactive Simulation:} We built an interactive simulator with a day-of-the-month slider. Users can scrub through the month to see how data availability impacts model readiness.
    \item \textbf{Heuristic Release Calendar:} Instead of relying on slow external APIs, the app uses a built-in heuristic calendar of expected release dates for indicators like CPI and Jobs Reports.
    \item \textbf{Live Health Scores:} As the slider moves, visual "Health Cards" update in real-time, showing a readiness percentage and progress bar for each model (AR, ADL, Random Forest, and the Combined Ensemble) based on the availability of their required features.
\end{itemize}

\begin{lstlisting}[language=JavaScript, caption=Ragged Edge Model Health Calculation]
const calculateModelHealth = (model: string) => {
  const dependencies = MODEL_DEPENDENCIES[model as keyof typeof MODEL_DEPENDENCIES];
  if (!dependencies) return 0;

  const releasedCount = dependencies.filter(feature => {
    const releaseDay = RELEASE_SCHEDULE[feature as keyof typeof RELEASE_SCHEDULE];
    return releaseDay <= simulatedDay;
  }).length;

  return Math.round((releasedCount / dependencies.length) * 100);
};
\end{lstlisting}
...
\subsection{UI \& Quality of Life Improvements}
\begin{itemize}
    \item \textbf{Smart Profile Dropdown:} Implemented a custom click-outside dismissal handler for the profile menu in the top navigation bar, matching standard modern web app behavior.
    \item \textbf{Scenario Context Cards:} When a user selects a historical scenario (e.g., "Global Financial Crisis (2008)"), a contextual explanation card dynamically appears above the KPIs to provide immediate background on the macroeconomic event.
    \item \textbf{Intelligent Defaults:} The application now defaults to a 10-year (10Y) lookback window on initial load and when returning to the "Live" scenario, preventing information overload while keeping the "MAX" view easily accessible.
    \item \textbf{Crash Prevention:} Added robust error handling and formatting fallbacks to prevent edge-case crashes in the Forward Outlook tab caused by empty data rows.
\end{itemize}

\begin{lstlisting}[language=JavaScript, caption=Click-Outside Dismissal Handler]
